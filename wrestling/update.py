...
class WrestlingDatabase:
    ...
    def process_multi_man_match(self, match_type, weight_class, fighter1_cell, result_cell,
                                 ...
                                 is_weekly=False, is_main_event=False):
        """Process a multi-man match (Tag, Trios, Three Way, Four Way, Battle Royal, etc.).
        ...
        for fighter in all_fighters:
            ...
            # Track main event appearances
            if is_main_event and not is_weekly:
                w['main_events'] += 1
                if 'wrestlemania' in event_name.lower():
                    w['wrestlemania_main_events'] += 1
                elif 'libremania' in event_name.lower():
                    w['libremania_main_events'] += 1

    @staticmethod
    def format_multi_man_main_event_text(multi_match):
        """Format a multi-man match as main event text for broadcast records.
        Three Way / Four Way: 'Santo vs. Demon vs. Mascaras'
        Trios: '<i>Relevos Australianos</i> Trios Match'
        Tag: '<i>Relevos Increíbles</i> Tag Match'
        Battle Royal / Royal Rumble / Ladder / Gauntlet: 'TYPE Match'
        """
        mt = multi_match['type'].lower()
        w_names = [f['name'] for f in multi_match.get('winners', [])]
        l_names = [f['name'] for f in multi_match.get('losers', [])]
        all_names = w_names + l_names

        if mt in ('three way', 'four way', 'triple threat', 'fatal four way'):
            return ' vs. '.join(all_names)
        elif mt == 'trios':
            return '<i>Relevos Australianos</i> Trios Match'
        elif mt == 'tag':
            return '<i>Relevos Increíbles</i> Tag Match'
        elif mt == 'royal rumble':
            return 'Royal Rumble Match'
        elif mt == 'gauntlet':
            return 'Gauntlet Match'
        elif mt == 'ladder':
            return 'Ladder Match'
        elif mt == 'battle royal':
            return 'Battle Royal Match'
        else:
            return f'{multi_match["type"]} Match'
    ...
    def _champion_orgs(self, champ, weight):
        """The sanctioning bodies whose belt this wrestler holds at this weight.
        The Ring is left out on purpose — it is decoration that tracks the lineal
        champion, it sanctions nothing and it gates no contendership."""
    ...
    # shot they earned never happens. Anything earlier is legacy: six contenders
    # from 2019 (WTS 6, 12 and 19) never cashed in under the old hand-booking,
        ...
        for n, rows in cards.items():
            ...
            for r in rows:
                ...
                if not any(k in r['note'] for k in ('championship', 'title')):
                    continue
                if r['weight'].lower() == wkey and name in (r['a'], r['b']):
                    return True
        return False
    ...
    def _collapse_unified(self, opmod, body):
        """Fold the defences of belts one man holds at one weight into a single
        match, the way a unified champion actually defends. WrestleMania puts
        every WWF belt on the line and the rotation may have another body's belt
        up the same night; if the same wrestler holds both he cannot appear
        twice, so the row is emitted once with a combined note."""
        out, seen = [], {}
        for mtype, weight, note in body:
            hit = opmod._row_belt(note, weight)
            if not hit or hit[1] != 'D':
                out.append((mtype, weight, note))
                continue
            org, wkey = hit[0][1].lower(), weight.lower()
            ...
            key = (wkey, champ[0])
            if key in seen:
                continue                     # already defending here tonight
            seen[key] = True
            out.append((mtype, weight,
                        self._title_note_for_champion(champ[0], wkey) or note))
        return out

    def _slot_event_block(self, opmod, summary, rows, info, day_offset=0,
                          owed=None):
        """One <details> card: template rows in, HTML out, champion and (where
        one has been earned) challenger filled.

        owed maps (weight, org) -> (country, name) for every contendership still
        ...
        for i, (mtype, weight, note) in enumerate(rows, 1):
            ...
            f1 = f2 = ('xx', '')
            if hit and hit[1] == 'D':
                wkey, row_org = weight.lower(), hit[0][1].lower()
                champ = self._current_champion(row_org, wkey)
                if champ:
                    f1 = (champ[1], champ[0])
                    for o in ({row_org} | self._champion_orgs(champ[0], wkey)):
                        if (wkey, o) in owed:
                            f2 = owed.pop((wkey, o))
                            break
                else:
                    # Belt vacant: contested, not defended — the same convention
                    # the World Title Series cards use. A contender owed a shot
                    # at it keeps his place and the other side goes by rating.
                    note = f"vacant {row_org.upper()} championship"
                    if (wkey, row_org) in owed:
                        f1 = owed.pop((wkey, row_org))
            built.append(opmod.wts_row(i, mtype, weight, note, f1=f1, f2=f2))
        if info:
            d = self.parse_date(info['date'])
            date_disp = ((d + timedelta(days=day_offset)).strftime('%B %-d, %Y')
                         if d else info['date'])
            info_row = opmod.card_info_row(date_disp, btype=info['btype'],
        ...
        return ("    <details>\n"
                ...
                f"{opmod.CARD_HEADER}{''.join(built)}{info_row}"
                "    </details>\n")

    def _generate_slot_event(self, path, opmod, raw, kind, slot):
        """Append whatever the calendar calls for when the slot is not a WTS."""
        try:
        ...
        if kind == 'trios':
            block = (f"\n<!-- {year} Trios -->\n"
                     + self._slot_event_block(opmod, f"{year} Trios Tournament",
                                              opmod.trios_rows(), info) + "\n")
            ...
            if kind == 'wrestlemania':
                ...
                name, tag = "LibreMania", f"{year} LibreMania"
            block = (f"\n<!-- {tag} -->\n"
                     + self._slot_event_block(opmod, f"{name} - Night 1: TBD", n1,
                                              info, owed=owed)
        ...
        with open(path, 'w', encoding='utf-8') as f:
            f.write(raw)
        print(f"Appended {label} (calendar slot {slot}).")
    ...
    def generate_next_wts_if_ready(self, path, opmod):
        ...
        soup = BeautifulSoup(raw, 'html.parser')
        highest = max(nums)
        if not opmod.last_event_is_complete(soup):
            return
        # The year is a fixed 32-slot calendar, so what comes next is whatever
        # the calendar says comes next — WrestleMania, LibreMania and the two
        # tournaments are generated exactly like a World Title Series is.
        slot = opmod.next_slot(soup)
        kind = opmod.slot_event_kind(slot)
        if kind != 'wts':
    ...
    def reprocess_championships_chronologically(self):
        ...
        # Find the most recent event date across ALL events (PPV and weekly)
        ...
        # Now update current champions' days to most recent event
        if most_recent_date:
            for org in self.championships:
                for weight in self.championships[org]:
                    reigns = self.championships[org][weight]
                    if reigns:
                        last_reign = reigns[-1]  # Current champion
                        if last_reign['date']:
                            start_date = self.parse_date(last_reign['date'])
                            if start_date:
    ...
    def parse_open_tournament_from_events(self):
        ...
        Finds singles matches where notes contain 'Open Tournament Finals'.
        Starting in 2020, events may specify 'Men's' or 'Women's' Open;
        if unspecified, defaults to men's.
        ...
        for event in self.events:
            for match in event.get('matches', []):
                ...
                # Determine gender: "Women's Open Tournament Finals" → women, else men
                notes_lower = notes.lower()
                if "women" in notes_lower:
                    gender = 'women'
                elif "men" in notes_lower:
                    gender = 'men'
                else:
                    gender = 'men'  # default if unspecified
    ...
    def parse_trios_tournament_from_events(self):
        """Derive Trios Tournament winners from already-parsed multi-man match data.
        Finds Trios matches where notes contain 'Trios Finals' (or 'Trios Tournament Finals').
        Each side has 3 wrestlers. Winners get trios_tournament_wins += 1.
        Stores list of dicts in self.tournaments['trios_with_years']."""
        self.tournaments['trios_with_years'] = []
        
        # Reset trios_tournament_wins for all wrestlers
        for w in self.wrestlers.values():
            w['trios_tournament_wins'] = 0
        
        for event in self.events:
            for mm in event.get('multi_man_matches', []):
                notes = mm.get('notes', '')
                if 'trios finals' not in notes.lower() and 'trios tournament finals' not in notes.lower():
                    continue
                
                winners = mm.get('winners', [])
                losers = mm.get('losers', [])
                if not winners:
                    continue
                ...
                self.tournaments['trios_with_years'].append({
                    'winners': winners,  # list of {name, country} dicts
                    'losers': losers,
                    'year': year,
                    'date': date_str,
                    'venue': event.get('venue', ''),
                ...
                for fighter in winners:
                    w = self.get_wrestler(fighter['name'])
                    w['trios_tournament_wins'] += 1
                    self.tournaments['trios'].append(fighter['name'])
        
        if self.tournaments['trios_with_years']:
            years = [t['year'] for t in self.tournaments['trios_with_years']]
            print(f"  Trios Tournament: found {len(self.tournaments['trios_with_years'])} editions from PPV data ({min(years)}–{max(years)})")

    def generate_open_tournament_html(self):
        """Generate the Open Tournament winners table for wiki.html from parsed data.
        When both men's and women's finals exist for the same year (same venue),
        they share a single row number with rowspan=2 on No./Year/Venue/Location."""
        entries = self.tournaments.get('open_with_years', [])
        if not entries:
            return ''
        
        # Sort by year descending
        entries_sorted = sorted(entries, key=lambda t: (-t['year'], t.get('gender', 'men')))
        
        # Group by year to detect shared rows
        ...
        # Number editions from total down to 1
        edition_num = len(year_groups)
        for year, group in year_groups.items():
            group_size = len(group)
            # Sort group: men first, then women
            group.sort(key=lambda t: 0 if t.get('gender', 'men') == 'men' else 1)
            
            for row_idx, t in enumerate(group):
                ...
                if row_idx == 0:
                    # First row in group — include No/Year/Venue/Location (with rowspan if group > 1)
    ...
    def generate_trios_tournament_html(self):
        ...
        for idx, t in enumerate(entries_sorted):
            ...
            html += f'                <th>{num}</th>\n'
            html += f'                <td>{t["year"]}</td>\n'
            html += f'                <td>{winners_html}</td>\n'
            html += f'                <td>{losers_html}</td>\n'
            html += f'                <td>{venue}</td>\n'
    ...
    def get_wrestler_titles(self, wrestler_name):
        ...
        # Include Trios Tournament wins
        trios_wins = [t for t in self.tournaments.get('trios_with_years', [])
                      if any(f['name'] == wrestler_name for f in t['winners'])]
        if trios_wins:
            for t in sorted(trios_wins, key=lambda x: x['year']):
                all_time[('trios', f'tournament_{t["year"]}')] = (f"{t['year']} Trios Tournament", 1)
    ...
    def generate_wrestler_page(self, wrestler_name):
        ...
        for idx, match in enumerate(sorted_matches):
            ...
            # Event cell: full event name on top, flag + location smaller underneath
            event_cell = match["event"]
            if match["location"]:
    ...
    def _plural(self, count, word):
        """Singularize a stat phrase when count == 1 (e.g. '1 wins' -> '1 win')."""
        try:
            n = int(count)
        ...
        if n != 1:
            return word
        if word == 'days as champion':
            return 'day as champion'
        if word.endswith('s'):
            return word[:-1]
        return word
    ...
    def generate_undisputed_champions_html(self):
        ...
        most_recent_date = None
        for event in self.events:
            event_date = self.parse_date(event.get('date'))
    ...
    def generate_streaks_records_html(self):
        ...
        for i in range(10):
            ...
            if i < len(top_cons_days):
                ...
                html += f'            <td><span class="fi fi-{stats["country"]}"></span> {self._wlink(name)}<br><span class="sub">{self.format_number(stats["max_days"])} {self._plural(stats["max_days"], "days as champion")}{self._years_suffix_for(name)}</span></td>\n'
            else:
                html += '            <td><span class="fi fi-xx"></span> Vacant<br><span class="sub">0</span></td>\n'
            ...
            if i < len(top_unique_opponents):
                ...
                html += f'            <td><span class="fi fi-{stats["country"]}"></span> {self._wlink(name)}<br><span class="sub">{len(stats["unique_opponents"])} {self._plural(len(stats["unique_opponents"]), "unique opponents")}{self._years_suffix_for(name)}</span></td>\n'
            else:
                html += '            <td><span class="fi fi-xx"></span> Vacant<br><span class="sub">0</span></td>\n'
        ...
        for i in range(10):
            ...
            for top_list, stat_key, word, pred in cats:
                if i < len(top_list):
                    ...
                    html += f'            <td><span class="fi fi-{w["country"]}"></span> {self._wlink(w["name"])}<br><span class="sub">{stat} {self._plural(stat, word)}{suffix}</span></td>\n'
                else:
                    html += '            <td><span class="fi fi-xx"></span> Vacant<br><span class="sub">0</span></td>\n'
    ...
    def generate_world_titles_records_html(self):
        ...
        for i in range(10):
            ...
            if i < len(top_reigns):
                ...
                html += f'            <td><span class="fi fi-{stats["country"]}"></span> {self._wlink(champ)}<br><span class="sub">{stats["total_reigns"]} {self._plural(stats["total_reigns"], "title reigns")}{self._years_suffix_for(champ)}</span></td>\n'
            else:
                html += '            <td><span class="fi fi-xx"></span> Vacant<br><span class="sub">0</span></td>\n'
            ...
            if i < len(top_defenses):
                ...
                html += f'            <td><span class="fi fi-{stats["country"]}"></span> {self._wlink(champ)}<br><span class="sub">{stats["total_defenses"]} {self._plural(stats["total_defenses"], "title defenses")}{self._years_suffix_for(champ)}</span></td>\n'
            else:
                html += '            <td><span class="fi fi-xx"></span> Vacant<br><span class="sub">0</span></td>\n'
            ...
            if i < len(top_days):
                ...
                html += f'            <td><span class="fi fi-{stats["country"]}"></span> {self._wlink(champ)}<br><span class="sub">{self.format_number(stats["total_days"])} {self._plural(stats["total_days"], "days as champion")}{self._years_suffix_for(champ)}</span></td>\n'
            else:
                html += '            <td><span class="fi fi-xx"></span> Vacant<br><span class="sub">0</span></td>\n'
            ...
            if i < len(top_title_bouts):
                ...
                html += f'            <td><span class="fi fi-{stats["country"]}"></span> {self._wlink(champ)}<br><span class="sub">{stats["title_bouts"]} {self._plural(stats["title_bouts"], "title bouts")}{self._years_suffix_for(champ)}</span></td>\n'
            else:
                html += '            <td><span class="fi fi-xx"></span> Vacant<br><span class="sub">0</span></td>\n'
...
def main():
    ...
    # Derive Trios Tournament winners from parsed multi-man match data
    print("Parsing Trios Tournament winners from PPV data...")
